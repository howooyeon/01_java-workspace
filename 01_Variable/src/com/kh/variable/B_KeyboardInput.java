...
public class B_KeyboardInput {
	...
	/* 정리
	 * 1. 콘솔창에 출력하기 위해  : System.out.println() 메소드 이용
	 * 2. 콘솔창에 입력받기 위해 : Scanner 이용해서 (nextLine(), next(), nextInt(), nextDouble...)
	 * -> 주의사항
	 * 1) sc.nextXXXX() 메소드뒤에 sc.nextLine() 메소드가 와야할 경우
	 * 	  sc.nextLine() 메소드를 한 번 더 써줘서 버퍼에 남아있는 '엔터'를 빼주는 과정 필수!
	 * 2) '문자' 값을 받아야 될 경우
	 * 	  sc.nextLine() 메소드를 통해 우선 문자열로 받고 그 뒤에 .charAt(인덱스 값) 메소드를 통해서 문자 하나 추출
	 * 
	 * */
